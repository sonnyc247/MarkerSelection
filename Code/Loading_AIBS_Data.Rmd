---
title: "Loading AIBS Data"
author: "Anukrati, Sonny (Edited for use)"
output: html_notebook
---

This document details the code for loading AIBS human brain data. The some code came from Anukrati Nigam, edited for Sonny Chen's use/the write-up of this notebook.
...
#download_dir = '/external/rprshnas01/netdata_kcni/stlab/AIBS_scRNAseq_2019/human/' #no writing permission, unfortunately
...
To proceed, we will need to unzip the tome file. [This is not run, because at the point of writing this markdown, the file has already been unzipped, with the zipped file deleted]

```{bash, unzip data}

unzip "/external/rprshnas01/kcni/ychen/git/MarkerSelection/Data/transcriptome.zip"
rm "/external/rprshnas01/kcni/ychen/git/MarkerSelection/Data/transcriptome.zip"

```

After this, we can load the metadata and see what our samples/cells our data contain. 
...
tome_sample_meta[1:10,c("sample_name", "region_label", "class_label", "cell_type_alias_label")]

```

Now we separate the clusters in the metadata into cell classes/subclasses/types to allow filtering.
...
As we can see above, each cell subclass only has one class value (except for outliers and donor-specific cell classes). These are some of the cells we will filter out; along with other filters/selections of the cells that we want.
