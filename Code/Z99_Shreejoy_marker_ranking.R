...
hgnc_mapping = read_tsv(url('http://ftp.ebi.ac.uk/pub/databases/genenames/hgnc/archive/monthly/tsv/hgnc_complete_set_2021-06-01.txt'))

# now, this is the list of sonnys markers with entrez ids and ensembl ids where possible
all_markers_ranked_w_ids = full_join(all_markers_ranked_w_entrez, 
...
AIBS_lnCPM_InEx_ACC <- t(as.data.frame(Seu_AIBS_obj[["RNA"]]@data[matching_inds, ])) %>% as.data.frame() #get transposed lnCPM matrix
...
write_csv(final_markers_df_ranked, file = '~/human_marker_genes/data/new_markers.csv')


### visualize a few cell type's markers using group dot plots

gdp_markers = final_markers_df_ranked %>% filter(subclass == 'SST', !is.na(ensembl_id)) %>% distinct(gene, .keep_all = T) %>%
