...
# i noticed that some of sonny's markers are missing ensembl ids, so i'm getting those from the raw data below ### where get these
...
all_markers_ranked_w_ids = left_join(all_markers_ranked_w_entrez, 
  ...
  dplyr::select(X1, gene, entrez_id, ensembl_id, everything()) ### just everything else?
...
AIBS_lnCPM_InEx_ACC <- t(as.matrix(Seu_AIBS_obj[["RNA"]]@data[matching_inds, ])) %>% as.data.frame() #get transposed lnCPM matrix ### changed to matrix
